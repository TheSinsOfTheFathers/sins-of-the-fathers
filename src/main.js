...
async function initializeHeavyModules() {
  try {
    ...
    if (dynamicModules.length > 0) {
      console.log(
        ` > Auto-Loader: Found ${dynamicModules.length} dynamic modules.`
      );
        ...
        if (moduleMap[targetPath]) {
          try {
            ...
            if (typeof module[fnName] === "function") {
              ...
              console.error(
                `Module method not found: ${fnName} in ${targetPath}`
              );
            }
          } catch (err) {
...
document.addEventListener("DOMContentLoaded", async () => {
  // Made async to await initI18n

  console.log(
    "%c TSOF // SYSTEM ONLINE ",
    "color: #000; background: #c5a059; padding: 5px; font-weight: bold; font-family: monospace; font-display: swap;"
  );
  ...
  const activeBtn = document.querySelector(
    `.lang-btn[data-lang="${currentLang.substring(0, 2)}"]`
  );
  if (activeBtn) {
  ...
  window.changeAppLanguage = async (lang) => {
    try {
      ...
      if (window.isMonitoringActive && window.SentryCaptureException)
        window.SentryCaptureException(e);
    }
  };
