...
function showMessage(target, msg, type = "info") {
  ...
  if (type === "error")
    target.className = "text-red-500 font-mono text-xs animate-pulse";
...
function firebaseErrorToMessage(err) {
  ...
  switch (code) {
    case "auth/email-already-in-use":
      return "Identity already exists in the database.";
    case "auth/invalid-email":
      return "Invalid communication frequency (Email).";
    case "auth/invalid-credential":
      return "Access Denied: Credentials rejected.";
    case "auth/weak-password":
      return "Security Alert: Key is too weak (Min 6 chars).";
    case "auth/user-not-found":
      return "Operative not found in archives.";
    case "auth/wrong-password":
      return "Encryption key mismatch (Wrong Password).";
    default:
...
export function initAuth() {
  ...
  const googleBtnRegister = document.getElementById(
  ...
  if (loginForm && registerForm && switchLogin && switchRegister) {
    ...
    const switchTo = (isRegister) => {
      const title =
        document.querySelector("h1") || document.getElementById("form-title");
      if (isRegister) {
  ...
  if (registerForm) {
    ...
    registerForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        setTimeout(
          () => (globalThis.location.href = "/pages/profile.html"),
          1000
        );
      } catch (err) {
        showPopup("error", firebaseErrorToMessage(err));
  ...
  onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
      ...
      document
        .querySelectorAll(".restricted-overlay")
        .forEach((overlay) => (overlay.style.display = "none"));
      document
        .querySelectorAll(".restricted-content")
        .forEach((content) =>
          content.classList.remove("restricted-content-blur")
        );
    } else {
      ...
      document
        .querySelectorAll(".restricted-overlay")
        .forEach((overlay) => (overlay.style.display = "flex"));
      document
        .querySelectorAll(".restricted-content")
        .forEach((content) => content.classList.add("restricted-content-blur"));
    }
  });
  ...
  function ensureUserMenu(user) {
    ...
    if (!controls) {
      console.error(
        " ! Critical: 'auth-controls' container not found. Cannot build menu."
      );
      return;
    }
