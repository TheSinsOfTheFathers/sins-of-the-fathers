  ...
  DOMPurify.sanitize(`
    <div class="gsap-archive-card opacity-0 archive-card bg-gray-900 border-l-4 border-gold p-5 shadow-lg h-fit break-inside-avoid group">
        ...
        <div class="flex items-center space-x-1 h-6 mb-4 opacity-60 group-hover:opacity-100 transition-opacity">
            ...
            <div class="w-1 bg-gray-700 h-px grow"></div>
        </div>
...
const createImageCard = (lore) => {
  ...
  cardDiv.className =
    "gsap-archive-card opacity-0 archive-card bg-white p-3 shadow-lg h-fit break-inside-avoid hover:rotate-1 transition-transform duration-300";

  cardDiv.innerHTML = DOMPurify.sanitize(`
...
const generateCard = (lore) => {
  ...
  switch (type) {
    case "audio":
      return createAudioCard(lore);
    case "image":
      return createImageCard(lore);
    default:
      return createDocumentCard(lore);
  }
};
...
const applyFilters = (searchTerm = "", filterType = "all", container) => {
  ...
  const filtered = allLoreData.filter((item) => {
    ...
    if (filterType === "documents")
      matchesType = item.loreType === "document" || !item.loreType;
...
export default async function (container, props) {
  ...
  try {
    ...
    try {
      const itemList = allLoreData.map((lore, index) => ({
        ...
        item: {
          ...
          url: new URL(
            `lore-detail.html?slug=${lore.slug}`,
    ...
    if (loader)
      loader.innerHTML = `<span class="text-red-500 font-mono">${i18next.t("lore_loader.system_error_short")}</span>`;
